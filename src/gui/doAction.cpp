...
void FurnaceGUI::doAction(int what) {
	...
	case GUI_ACTION_STOP:
		...
		edit = false;
		break;
	case GUI_ACTION_RECORD_PATTERN:
		edit = true;
		play();
		break;
	...
	case GUI_ACTION_OCTAVE_DOWN:
		...
		break;
	case GUI_ACTION_OCTAVE1:
		curOctave = 0;
		e->autoNoteOffAll();
		failedNoteOn = false;
		break;
	case GUI_ACTION_OCTAVE2:
		curOctave = 1;
		e->autoNoteOffAll();
		failedNoteOn = false;
		break;
	case GUI_ACTION_OCTAVE3:
		curOctave = 2;
		e->autoNoteOffAll();
		failedNoteOn = false;
		break;
	case GUI_ACTION_OCTAVE4:
		curOctave = 3;
		e->autoNoteOffAll();
		failedNoteOn = false;
		break;
	case GUI_ACTION_OCTAVE5:
		curOctave = 4;
		e->autoNoteOffAll();
		failedNoteOn = false;
		break;
	case GUI_ACTION_OCTAVE6:
		curOctave = 5;
		e->autoNoteOffAll();
		failedNoteOn = false;
		break;
	case GUI_ACTION_OCTAVE7:
		curOctave = 6;
		e->autoNoteOffAll();
		failedNoteOn = false;
		break;
	...
	case GUI_ACTION_PAT_PASTE_OVERFLOW:
		...
		break;
	case GUI_ACTION_PAT_TRACK_CUT: {
		SelectionPoint selEndPat;
		selEndPat.xFine = 2 + e->curPat[selEndPat.xCoarse].effectCols * 2;
		selEndPat.y = e->curSubSong->patLen - 1;
		doCopy(true, true, SelectionPoint(0, 0, 0), selEndPat);
		break;
	}
	case GUI_ACTION_PAT_TRACK_COPY: {
		SelectionPoint selEndPat;
		selEndPat.xFine = 2 + e->curPat[selEndPat.xCoarse].effectCols * 2;
		selEndPat.y = e->curSubSong->patLen - 1;
		doCopy(false, true, SelectionPoint(0, 0, 0), selEndPat);
		break;
	}
	case GUI_ACTION_PAT_TRACK_PASTE:
		doPaste();
		break;
	case GUI_ACTION_PAT_TRACK_SELECT:
		selStart.y = 0;
